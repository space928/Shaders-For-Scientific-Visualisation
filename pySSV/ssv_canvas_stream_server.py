...
class SSVCanvasStreamServer:
    ...
    def __init__(self, http: bool = False, port: Optional[int] = None):
    ...
    def _run_server(self):
        ...
        if self._http:
            handler = partial(SSVCanvasStreamServerHTTP, self._msg_queue, lambda: self._is_alive)
            with HTTPServer((self._hostname, self._port), handler) as server:
                self._server = server
                self._server.serve_forever()
        else:
            with serve(self._on_connect, self._hostname, self._port) as server:
                self._server = server
                self._is_alive = True
